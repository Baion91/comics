# Handoff — cập nhật lần cuối: 2026-09-01 (comix TRÁO Ô: đọc cờ s:1 + giải-xáo qua canvas site + chế độ --repair-scramble + detector seam)
...
## Đang làm / dở dang
- **[01/09] comix — ảnh Official lỗi định kỳ do TRÁO Ô: ĐÃ code 3 phần + compile/import OK + detector xác minh offline trên file xáo thật; CHƯA nghiệm thu LIVE giải-xáo trên server** (`comix_site.py`, `comics_core.py`, `comic_downloader.py`, `check_library.py`).
  ...
  **Fix 3 phần** (chi tiết + lý do ở ARCHITECTURE mục comix_site + memory `comix-scramble-s-flag`): **①** `fetch_pages` trả `[{"url","s"}]`; `run()` tách trang thường (HTTP như cũ) vs trang `s:1` → `ComixSession.descramble_pages()` chạy `DESCRAMBLE_JS` (`page.evaluate`): `import(secure.js)` (dò URL động qua Performance API), `vs(url).apply(canvas)` trên canvas ẩn, chờ vẽ, `toDataURL` lấy bytes sạch (canvas KHÔNG taint vì wowpic CORS mở). KHÔNG reverse permutation (secure.js obfuscate nặng). **②** `--repair-scramble` (comix): dò offline từng chương (bỏ nhanh chương sạch, không chạm mạng), chương còn ảnh xáo → lấy payload bản đang có trên đĩa (khớp `chapterId` sidecar / hoặc đòi khớp số trang) → giải-xáo lại đúng trang `s:1`, ghi đè + đóng lại `.done`; KHÔNG tải chương mới. **③** `comics_core.looks_scrambled()` (đo seam-energy biên ô bằng PIL, ngưỡng 4.0, không numpy): dùng làm GUARD trong `run()` (trang `s:1` còn dấu xáo = chưa xong → không `.done` → tự bù) + trigger cho repair + audit trong `check_library.py` (mục "trang tráo ô", gợi ý chạy repair).
  **Đã kiểm**: `py_compile` cả 4 file OK; import module OK; **detector chạy thật**: 9 file xáo (010–090) ratio 8.6–22 vs 5 file sạch 1.1–1.7 → ngưỡng 4.0 tách chắc; `check_library "downloads\Farmer of Spirits" --recheck` báo đúng 9 trang tráo ô. **Mắt xích browser đã xác minh** (trong Browser pane): `vs`=export `t` import được + trả `{apply}`; `apply` gọi `drawImage` từ `ImageBitmap` (lỗi "detached" ở lần gọi 2 = lần 1 đã vẽ); canvas không taint. **CHƯA xác minh được PIXEL giải-xáo tại máy dev**: Browser pane ẩn không compositing → mọi readback canvas (kể cả OffscreenCanvas) trả rỗng — giới hạn môi trường, KHÔNG phải lỗi cơ chế. → **cần chạy thử trên server (Chromium headful hiển thị) để xác nhận**.
  **Nghiệm thu LIVE cần làm**: chạy `--repair-scramble` cho 1 bộ (giới hạn `--chapters` cho an toàn) khi màn hình server đang mở/không khóa → mở ảnh trang 10/20… xem đã sạch; guard đảm bảo giải-xáo hụt chỉ báo "còn sót" (an toàn, không hỏng dữ liệu, không nhận nhầm .done).
  **Lệnh bot `/repair`** (thêm 01/09, trong `supervisor.py`): `/repair <link comix> [chương]` đưa job `--repair-scramble` vào ĐÚNG hàng đợi tải (dedup theo (url,chapters,repair) nên không đè job /tai; nhãn 🧩 trong /trangthai; báo "🧩 Bắt đầu SỬA TRÁO Ô"/"✅ Sửa tráo ô xong"). `comix_site` in progress mỗi chương trong repair để log luôn tiến (tránh stall-watchdog 1200s kill oan khi quét nhiều chương sạch). Vẫn dùng được CLI trực tiếp `python comic_downloader.py <url> --repair-scramble`.
  **Deploy**: push (`day-len.bat`) → trên server `/update` (bot) kéo code, NHƯNG lần này **`supervisor.py` đã đổi → phải RESTART supervisor tay** (chạy lại `server-BAT-tudong.bat`, hoặc tắt/bật) để nạp lệnh `/repair`; `/update` sẽ tự nhắc "⚠️ supervisor.py đã đổi". (comix_site/comics_core/comic_downloader chạy tiến trình mới mỗi job nên tự nạp; chỉ supervisor cần restart.) Test trước cho chắc: `/repair <link> 1` (1 chương) rồi mở ảnh trang 10 xem sạch → ổn thì `/repair <link>` cả bộ.
...
- **[24/08] Giữ supervisor SỐNG (Hướng A) + vá bug Service Worker "Returned response is null" — ĐÃ code + compile/parse OK + push, CHƯA nghiệm thu LIVE trên server** (`supervisor.py`, `reader_server.py`, `watchdog.ps1` [mới], `server-BAT-tudong.bat`, `server-TAT-tudong.bat`, `notify-config.example.json`).
