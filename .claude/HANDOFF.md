...
## Đang làm / dở dang
- **[02/09] comix — ảnh Official lỗi định kỳ do TRÁO Ô: ĐÃ code + KIỂM CHỨNG end-to-end offline (dựng lại trang sạch bằng PIL); nên chạy 1 lần trên server cho chắc** (`comix_site.py`, `comics_core.py`, `comic_downloader.py`, `check_library.py`, `supervisor.py`).
  *Đổi hướng giải-xáo 02/09*: bản canvas-`toDataURL` (01/09) **THẤT BẠI trên server** (0 trang) — setup này đọc pixel canvas ra RỖNG (F12: `[[940,1388,114]]`) dù màn hình thấy sạch (nghi chống-scrape chặn readback; reader chỉ hiển thị nên không sao). ĐÃ thay bằng: **ghi bản đồ ô (chặn drawImage) + xếp lại bằng PIL offline** — xem ①.
  ...
  **Đã kiểm**: `py_compile` cả 5 file OK; import + chữ ký OK. **Ghi bản đồ ô chạy được ngay trong pane ẩn KHÔNG compositing** (bắt được 26 lệnh: 1 nền + 25 ô 5×5 188×277 cho trang 10 ch.1). **Dựng lại ảnh SẠCH end-to-end**: chạy đúng `comix_site._unscramble_ops()` trên bytes xáo thật → ra trang 10 ch.1 sạch hoàn toàn ("I GAVE UP MY DREAM…"). 2 fetch cùng URL trùng hash (scramble ổn định theo URL) — nhưng file cũ `010.webp` khác hash bản hiện tại (token đổi) → **file cũ KHÔNG giải được**, repair phải tải lại (đằng nào cũng re-fetch). Detector: 9 file xáo ratio 8.6–22 vs sạch 1.1–1.7.
  **Vì sao hướng mới bền hơn**: ghi tham số drawImage KHÔNG cần compositing/đọc canvas → chạy được cả khi **server không màn hình / RDP ngắt**; pixel do PIL lo (chắc chắn) → **kiểm chứng offline được**. (Bản cũ cần cửa sổ vẽ + readback không bị chặn — cả 2 điều kiện setup này không đáp ứng.)
  **Nên làm 1 lần trên server**: `/repair <link> 1` → mở trang 10 xem sạch; guard đảm bảo giải-xáo hụt chỉ báo "còn sót" (an toàn). Ổn thì `/repair <link>` cả bộ.
